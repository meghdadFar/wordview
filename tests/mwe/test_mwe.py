...
@pytest.fixture
def dummy_text_panda_with_a_noun_compund():
    text = [
...
@pytest.fixture
def dummy_text_panda_with_no_noun_compund():
    text = [
...
class TestMweInitialisation:
    ...
    def test_mwe_does_not_tokenize_text_with_multiple_whitespaces(self, dummy_text_panda):
    ...
    def test_mwe_tokenizes_text_with_multiple_whitespaces(self, dummy_text_panda):
        mwe = MWE(df = dummy_text_panda, text_column = "text", tokenize=True)
        assert mwe.df["text"][0] == "new york is the capital of new york state"


    def test_mwe_with_wrong_mwe_type_raises_value_error(self, dummy_text_panda):
        with pytest.raises(ValueError):
            mwe = MWE(df = dummy_text_panda, text_column = "text", mwe_types = ["XXX"])


    def test_mwe_with_empty_mwe_type_raises_value_error(self, dummy_text_panda):
        with pytest.raises(ValueError):
...
class TestMweCounter:
    ...
    def test_mwe_if_nc_present_returns_counts_with_nc(self, dummy_text_panda_with_a_noun_compund):
        mwe = MWE(df = dummy_text_panda_with_a_noun_compund, text_column = "text", tokenize=True, mwe_types = ["NC"])
    ...
    def test_mwe_if_no_nc_returns_empty_mwe_counts(self, dummy_text_panda_with_no_noun_compund):
        mwe = MWE(df = dummy_text_panda_with_no_noun_compund, text_column = "text", tokenize=True, mwe_types = ["NC"])
        counts = mwe.get_counts()
        assert counts["NC"] == {}


@pytest.mark.xfail
def test_mwe_build_counts(dummy_text_panda_with_no_noun_compund):
    mwe = MWE(df = dummy_text_panda_with_no_noun_compund, text_column = "text", tokenize=True, mwe_types = ["NC"])
    counts = mwe.get_counts()
    assert False
