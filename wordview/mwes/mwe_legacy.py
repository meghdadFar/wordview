    ...
    def _check_tokenized(self) -> None:
        ...
        if float(num_pass) / float(len(tests)) < 0.8:
            logger.warning(
                ...
                \nThis can lead to poor results. Consider re-instantiating your MWE instance with 'tokenize' \
    ...
    def extract_mwes(
        ...
        if counts:
            ...
            try:
                ...
                logger.error(e)
                logger.error(
                    "Counts must be provided either via input argument `counts` or `counts_filename`. \
                    Argument `counts` is not specified and it seems like there was an error reading the counts from `counts_filename`."
                )
                raise e
        else:
